...
router.post('/check-delivery', AddressController.checkDeliveryAvailability);
...
router.get('/user', authenticateToken, AddressController.getUserAddresses);
...
router.get('/user/with-delivery', authenticateToken, AddressController.getUserAddressesWithDelivery);

// GET /api/addresses/user/:id - Получить конкретный адрес пользователя
// Headers: { "Authorization": "Bearer <token>" }
// Params: id (number) - ID адреса
router.get('/user/:id', authenticateToken, AddressController.getUserAddressById);
...
router.post('/user', authenticateToken, AddressController.createUserAddress);

// PUT /api/addresses/user/:id - Обновить адрес
// Headers: { "Authorization": "Bearer <token>" }
// Params: id (number) - ID адреса
// Body: { "name": "Новое название", "apartment": "26" } (любые поля для обновления)
router.put('/user/:id', authenticateToken, AddressController.updateUserAddress);

// DELETE /api/addresses/user/:id - Удалить адрес (мягкое удаление)
// Headers: { "Authorization": "Bearer <token>" }
// Params: id (number) - ID адреса
router.delete('/user/:id', authenticateToken, AddressController.deleteUserAddress);
